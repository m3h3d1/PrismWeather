...
public class UserService {
    ...
    public UserProfileResponse getCurrentUser(String email) {
        ...
        if(profile == null) {
            profile = profileRepository.findById(user.getId())
                    .orElseThrow(() -> new CustomException("Profile not found", HttpStatus.NOT_FOUND.value()));
        }
     ...
     * @param email      Email of the currently authenticated user (to ensure authorization).
    ...
    public UserProfileResponse updateUserProfile(Long id, String email, UpdateUserRequest requestDto) {
        ...
        if (!user.getEmail().equals(email)) {
            throw new CustomException("You are not authorized to edit this profile", HttpStatus.FORBIDDEN.value());
        }

        Profile profile = profileRepository.findById(user.getId())
                .orElseThrow(() -> new CustomException("Profile not found", HttpStatus.NOT_FOUND.value()));

        if (requestDto.getFirstName() != null) profile.setFirstName(requestDto.getFirstName());
        if (requestDto.getLastName() != null) profile.setLastName(requestDto.getLastName());
        if (requestDto.getPhoneNumber() != null) profile.setPhoneNumber(requestDto.getPhoneNumber());
        if (requestDto.getCountry() != null) profile.setCountry(requestDto.getCountry());
        if (requestDto.getOccupation() != null) profile.setOccupation(requestDto.getOccupation());
        if (requestDto.getAddress() != null) profile.setAddress(requestDto.getAddress());
        if (requestDto.getDateOfBirth() != null) profile.setDateOfBirth(LocalDate.parse(requestDto.getDateOfBirth()));
